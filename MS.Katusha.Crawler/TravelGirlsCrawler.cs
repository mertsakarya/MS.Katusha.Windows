...
namespace MS.Katusha.Crawler
{
    public class TravelGirlsCrawler : ICrawler
    {
        ...
        {
            Sex gender;
            if (parameters.Length != 2) throw new ArgumentException("Need two values: First Gender second is page number");
        ...
        private static string ProcessProfile(AdminExtendedProfile profile, IEnumerable<HtmlNode> infos, IEnumerable<HtmlNode> images)
        {
            ...
            //using (var file = new StreamWriter(String.Format(KatushaFolder + @"\TravelGirls\{0}.json", profile.User.UserName))) {
            //    file.Write(str);
            //}
            return str;
        }
